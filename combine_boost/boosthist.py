...
parser.add_argument(
    "--nThreads", type=int, help="number of threads", default=None
)

## only applied if rebin
parser.add_argument(
    "--min-yield", dest="min_yield", type=float, required=True, help=""
)

parser.add_argument(
    "--mcstatmin", dest="min_mcstat", type=float, required=True, help=""
)
...
if __name__ == "__main__":
    ...
    perform_operations(available_cpus,
        sum_np_rebinned,
        sumraw_np_rebinned,
        min_yield_value,
        min_mc_value
    ...
    for h in rhists:
        ...
        print(
            "removing {} zeroes on {} ...".format(
                len(zero_indices), h.GetName()
            )
        )
        ...
        splitted_sumerr_np_hist = np.array_split(
            procerr_nozeroes_np, available_cpus
        )
        ...
        print(
            "final number of bins in {}: {}".format(
                hname + "_rebin", rhist.GetNbinsX()
            )
        )
